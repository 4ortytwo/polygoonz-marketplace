...
import '../interfaces/IPolygoonz.sol';
import '../interfaces/IPolygoonzMetadata.sol';

contract Polygoonz is ERC721Enumerable, Ownable, IPolygoonz, IPolygoonzMetadata {
  using Strings for uint256;

  uint256 public constant POLYGOONZ_GIFT = 100;
  uint256 public constant POLYGOONZ_PUBLIC = 8000;
  uint256 public constant POLYGOONZ_MAX = POLYGOONZ_GIFT + POLYGOONZ_PUBLIC;
  uint256 public constant PURCHASE_LIMIT = 8;
  ...
  constructor()  ERC721("Polygoonz", "PLGNZ") {}
  ...
  */
  ...
  function purchase(uint256 numberOfTokens) external override payable {
    ...
    require(totalSupply() < POLYGOONZ_MAX, 'All tokens have been minted');
    ...
    */
    require(totalPublicSupply < POLYGOONZ_PUBLIC, 'Purchase would exceed POLYGOONZ_PUBLIC');
    require(PRICE * numberOfTokens <= msg.value, 'ETH amount is not sufficient');

    for (uint256 i = 0; i < numberOfTokens; i++) {
      /**
      * @dev Since they can get here while exceeding the POLYGOONZ_MAX,
      * we have to make sure to not mint any additional tokens.
      */
      if (totalPublicSupply < POLYGOONZ_PUBLIC) {
        /**
        * @dev Public token numbering starts after POLYGOONZ_GIFT.
        * And we don't want our tokens to start at 0 but at 1.
        */
        uint256 tokenId = POLYGOONZ_GIFT + totalPublicSupply + 1;
  ...
  function purchaseAllowList(uint256 numberOfTokens) external override payable {
    ...
    require(totalPublicSupply + numberOfTokens <= POLYGOONZ_PUBLIC, 'Purchase would exceed POLYGOONZ_PUBLIC');
  ...
  function gift(address[] calldata to) external override onlyOwner {
    require(totalSupply() < POLYGOONZ_MAX, 'All tokens have been minted');
    require(totalGiftSupply + to.length <= POLYGOONZ_GIFT, 'Not enough tokens left to gift');
